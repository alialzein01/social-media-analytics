...
def build_pdf_report(
    ...
    story.append(Paragraph(f"<b>Generated:</b> {datetime.now().strftime('%Y-%m-%d %H:%M')}", normal))
    if date_range_str:
    ...
    try:
        ...
        get_post_engagement = lambda p, _: (p.get("likes", 0) + p.get("comments_count", 0) + p.get("shares_count", 0))
    ...
    story.append(Paragraph(_get_report_footer(), ParagraphStyle(name="Footer", parent=normal, fontSize=8, textColor=colors.grey)))
