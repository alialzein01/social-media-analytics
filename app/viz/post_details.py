...
def create_enhanced_post_selector(posts: List[Dict], platform: str) -> Optional[Dict]:
    ...
    for i, post in enumerate(posts):
        ...
        posts_with_scores.append({"index": i, "post": post, "engagement": engagement})
    ...
    if sort_order == "Most Recent":
        filtered_posts.sort(key=lambda x: x["post"].get("published_at", datetime.min), reverse=True)
...
def create_post_performance_analytics(post: Dict, all_posts: List[Dict], platform: str) -> None:
    """
    ...
    avg_comments = (
        sum(p.get("comments_count", 0) for p in all_posts) / len(all_posts) if all_posts else 0
    )
    avg_shares = (
        sum(p.get("shares_count", 0) for p in all_posts) / len(all_posts) if all_posts else 0
    )
    ...
    with col1:
        ...
        st.metric("Likes", f"{likes:,}", delta=f"{likes_delta:+.1f}% vs avg", delta_color="normal")
    ...
    with col3:
        ...
        st.metric(
            ...
            f"{shares:,}"
            if platform != "Instagram"
            else f"{(post_engagement / max(post.get('likes', 1), 1) * 100):.1f}%",
    ...
    with col4:
        engagement_delta = (
            ((post_engagement - avg_engagement) / avg_engagement * 100) if avg_engagement > 0 else 0
        )
        st.metric(
            "Total Engagement",
            f"{post_engagement:,}",
    ...
    if platform == "YouTube":
        ...
        with col1:
            ...
            st.metric("Views", f"{views:,}", delta=f"{views_delta:+.1f}% vs avg")
        ...
        with col2:
            ...
            st.metric("Engagement Rate", f"{engagement_rate:.2f}%", delta=f"{eng_rate_delta:+.2f}%")
    ...
    # Rank this post
    engagements = [
        (i, p.get("likes", 0) + p.get("comments_count", 0) + p.get("shares_count", 0))
        for i, p in enumerate(all_posts)
    ]
    engagements.sort(key=lambda x: x[1], reverse=True)

    post_rank = next(
        (i + 1 for i, (idx, _) in enumerate(engagements) if all_posts[idx] == post), None
    )
...
def create_comment_analytics(post: Dict, platform: str) -> None:
    ...
    if not comments_list or (isinstance(comments_list, int)):
        st.info(
            "💡 No detailed comments available. Enable 'Fetch Detailed Comments' to analyze comment text."
        )
        return
    ...
    for comment in comments_list:
        if isinstance(comment, dict):
            ...
            comment_data.append({"text": comment, "author": "Unknown", "timestamp": "", "likes": 0})
